...
public void OnPluginStart()
{
	...
	LoadTranslations("rockthevote_spec.phrases");
	...
	g_Cvar_IgnoreSpec = CreateConVar("sm_rtv_ignore_spec", "0", "Ignore spectator RTV requests", _, true, 0.0, true, 1.0);
...
void AttemptRTV(int client, bool isVoteMenu=false)
{
	if (g_Cvar_IgnoreSpec.BoolValue && IsClientObserver(client))
	{
		...
		{
			ReplyToCommand(client, "[SM] %t", "RTV Spectator");	
		}
		return;
	}
