...
public void OnPluginStart() {
	...
	ItemDefine("Tomislav", "tomislav", "Reverted to pre-pyromania, 40% faster spinup, no accuracy bonus, no barrel spin sound, 20% slower firing speed", CLASSFLAG_HEAVY, Wep_Tomislav, 1);
	ItemVariant(Wep_Tomislav, "Reverted to release, 75% faster spinup, no accuracy bonus, no barrel spin sound, 20% slower firing speed", 1);
...
public Action TF2Items_OnGiveNamedItem(int client, char[] class, int index, Handle& item) {
	...
	switch (index) {
		...
		case 588: { if (ItemIsEnabled(Wep_Pomson)) {
			item1 = TF2Items_CreateItem(0);
			TF2Items_SetFlags(item1, (OVERRIDE_ATTRIBUTES|PRESERVE_ATTRIBUTES));
			bool releaseVer = GetItemVariant(Wep_Pomson) == 2;
			TF2Items_SetNumAttributes(item1, 1);
		...
		case 424: { if (ItemIsEnabled(Wep_Tomislav)) {
			item1 = TF2Items_CreateItem(0);
			TF2Items_SetFlags(item1, (OVERRIDE_ATTRIBUTES|PRESERVE_ATTRIBUTES));
			bool releaseVer = GetItemVariant(Wep_Tomislav) == 2;
			TF2Items_SetNumAttributes(item1, 2);
...

			{
				...
				if (
					...
					if(GetItemVariant(Wep_Pickaxe) == 1) // Pre-Pyromania Equalizer (pre-June 27, 2012); 107 dmg at 1 hp
						...
						damage = (damage * ValveRemapVal(float(health_cur), 0.0, float(health_max), 1.76, 0.5));
...

				{
					...
					if (StrEqual(class, "tf_projectile_energy_ring")) {
						...
						if (
							...
							if (StrEqual(class, "tf_weapon_raygun") || (GetItemVariant(Wep_Pomson) && StrEqual(class, "tf_weapon_drg_pomson"))) {
								pos1[2] = 0.0;
								pos2[2] = 0.0;
